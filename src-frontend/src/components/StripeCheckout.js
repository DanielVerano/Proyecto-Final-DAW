...
const CheckoutForm = () => {
  ...
  const createPaymentIntent = async () => {
    try {
      const { data } = await axios.post(
        `${apiUrl}/orders/create-payment-intent`,
  ...
  return (
    <div>
      {succeeded ? <article className='d-flex justify-content-center text-center'>
        <div>
          <h4>Gracias!</h4>
          <h4>Tu pago se ha realizado correctamente!</h4>
        </div>
      </article>
        : <article>
          <h4>Hola, {myUser && myUser.name}</h4>
          <p>El total de su pedido es {formatPrice(shipping_fee + total_amount)}</p>
        </article>}
